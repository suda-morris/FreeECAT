/**
 * EtherCAT Master demo based on IgH, just for test!
 */
#include <errno.h>
#include <signal.h>
#include <stdio.h>
...
static uint16_t sensor_temp;

static ec_master_t *master = NULL;  /* manage slaves, domains and io */
static ec_master_state_t master_state = {}; /* restore the current state of master */

static ec_domain_t *domainOutput = NULL;    /* handles output process data and the needed datagrams of a certain group of slaves */
static ec_domain_state_t domainOutput_state = {};   /* restore the latest state of domainOutput */
static ec_domain_t *domainInput = NULL; /* handles input process data and the needed datagrams of a certain group of slaves */
static ec_domain_state_t domainInput_state = {};    /* restore the latest state of domainInput */
...
static uint8_t *domainOutput_pd = NULL; /* process date pointer for output */
static uint8_t *domainInput_pd = NULL;  /* process date pointer for input */

#define XMC_SlavePos 0, 0   /* slave alias, slave position */

#define Infineon_XMC4800 0x0000034E, 0x00000000 /* vendor ID, product code */

static unsigned int off_sensors_in;  /* offsets for PDO entries: sensor input */
static unsigned int off_leds_out;   /* offsets for PDO entries: leds output */

static unsigned int counter = 0; 
static unsigned int blink = 0;

/**
 * following parameters can be obtained from command: ethercat cstruct
 */
...
static void cyclic_task()
{
    ...
    if (counter) {
        ...
        sensor_temp = EC_READ_U16(domainInput_pd + off_sensors_in);
        ...
        EC_WRITE_U8(domainOutput_pd + off_leds_out, blink);
    }
...
int main(int argc, char **argv)
{
    ...
    /* creates a new process data domain for input */
    domainInput = ecrt_master_create_domain(master);
    if (!domainInput){
        return -1;
    }
    /* obtains a slave configuration */
    sc_xmc = ecrt_master_slave_config(master,XMC_SlavePos,Infineon_XMC4800);
    if (!sc_xmc) {
    ...
    off_leds_out = ecrt_slave_config_reg_pdo_entry(sc_xmc, 0x7000, 1, domainOutput, NULL);
    if(off_leds_out < 0){
        fprintf(stderr, "PDO entry registration failed!\n");
        return -1;
    }
    off_sensors_in = ecrt_slave_config_reg_pdo_entry(sc_xmc, 0x6000, 1, domainInput, NULL);
    if(off_sensors_in < 0){
        fprintf(stderr, "PDO entry registration failed!\n");
        return -1;
    }
